...
def get_info(host='localhost', port=25565):
    ...
    # Example response: (http://wiki.vg/Server_List_Ping)
    #
    # {
    # "version": {
    #     "name": "1.7.9",
    ...
    # }
def get_info_dict(host='localhost', port=25565):
    print "Conecting to %s:%s" % (host, port)

    server_info = get_info(host, port)

    players = []

    players_list = server_info['players'].get('sample')
    ...
    d = {
            'motd': str(server_info['description']),
            'version': str("Minecraft " + server_info['version']['name']),
            'online_players': server_info['players']['online'],
